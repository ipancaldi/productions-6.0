...
export function seedFigures(takeId) {
  if (!figuresOf || !takeId || takeId in lastFigures) return;
  lastFigures[takeId] = figuresOf(takeId);
}
...
export function record(kind, d = {}) {
  ...
  const last = fin
    ? [...LOG].reverse().find(e => e.kind === kind && e.who === who_ && e.takeId === d.takeId
                                  && e.obj === d.obj && e.step === d.step && e.task === (d.task || null)
                                  && now - e.at < win)
    : LOG[LOG.length - 1];
  ...
  const e = { n: ++seq, at: now, who: d.who || who.me, prodId: d.prodId || null,
              takeId: d.takeId || null, task: d.task || null, kind,
...
export const changes = computed(() => {
  ...
  LOG.forEach(e => {
    ...
    if (c && c.who === e.who && c.takeId === e.takeId && c.task === e.task
        && e.at - c.to_at < GAP_MS) {
      ...
      out.push({ id: 'c' + e.n, n: e.n, who: e.who, prodId: e.prodId, takeId: e.takeId,
                 ...
                 before: e.before, after: e.snap, state: e.state });
    }
  });
  /* ---- WHICH ROUTE IS LIVE -------------------------------------------
...
export function reqsOfChange(c) {
  const set = new Set();
  c.entries.forEach(e => { if (e.obj && e.obj.includes('-')) set.add(e.obj.split('-')[0]); });
